...
Feature: Autenticación de cliente con DNI

  @AuthenticationClientEnrolled
  Scenario Outline: Autenticación exitosa de un cliente DNI desde las apis de identity.
    ...
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion "<statusAuth>"
    ...
    Examples:
      | numeroDocumento | estado   | canal         | statusAuth | descripcion                     | estado |
      | 43729336        | ENROLLED | CANAL_EXTERNO | Positive   | Positive authentication result. | ok     |
      | 73006406        | ENROLLED | CANAL_EXTERNO | Positive   | Positive authentication result. | ok     |
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error por "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error debido a que en la imagen del selfie el cliente esta con "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error porque en el selfie existen "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
    And el contador de bloqueo de autenticacion del cliente con DNI "<numeroDocumento>" no aumenta
  ...
  el código de error no se contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error con el selfie con "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
    And el contador de bloqueo de autenticacion del cliente con DNI "<numeroDocumento>" no aumenta
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error debido a que el selfie tiene "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
    And el contador de bloqueo de autenticacion del cliente con DNI "<numeroDocumento>" no aumenta
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error debido a que el selfie no tiene "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
    And el contador de bloqueo de autenticacion del cliente con DNI "<numeroDocumento>" no aumenta
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error debido a que en la foto del selfie se detecto un "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
    And el contador de bloqueo de autenticacion del cliente con DNI "<numeroDocumento>" no aumenta
  ...
  contabiliza como un motivo de bloqueo.
    ...
    And el cliente con "<numeroDocumento>" se encuentra "<estado>" y se puede autenticar
    And el cliente con "<numeroDocumento>" no se encuentra bloqueado en la autenticacion
    When el cliente con numero de documento "<numeroDocumento>" envia una peticion de autenticacion con error debido a que se detecto un rostro "<estado1>"
    Then se valida que el cliente con DNI "<numeroDocumento>" tiene como respuesta de autenticacion el codigo de error  "<codigoError>" con mensaje de error "<mensajeError>"
    And el contador de bloqueo de autenticacion del cliente con DNI "<numeroDocumento>" no aumenta
